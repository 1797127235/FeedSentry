...
未设置 `FEEDSENTRY_MCP_TOKEN` 时，MCP 完全关闭，访问 `/mcp` 会返回 404。
...
反向代理必须保留 `Authorization` 请求头。大多数代理默认会保留；如果使用了额外鉴权插件，需要确认 Bearer Token 没有被移除。
...
保持服务器为启用状态，点击“测试连接”。连接成功后应当显示 FeedSentry 的 13 个工具，然后保存配置。
